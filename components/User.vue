  ...
  >
    <template #activator="{ on, attrs }" v-if="$auth.loggedIn">
      ...
      >
        <v-avatar left class="mr-3" size="28">
          <img :src="$auth.user.avatar_url" alt="User's profile picture" />
        </v-avatar>
        {{ $auth.user.login }}
      </v-btn>
    </template>
    <v-card max-width="400">
      <v-list>
        <v-list-item>
          <v-list-item-avatar size="80">
            <img :src="$auth.user.avatar_url" alt="User's profile image" />
          </v-list-item-avatar>
          <v-list-item-content>
            <v-list-item-title class="text--h6 font-weight-bold ml-3 mb-1">{{
              $auth.user.login
            }}</v-list-item-title>
            <v-list-item-subtitle>
              ...
              >
                ...
                {{ $auth.user.id }}
                <v-icon x-small right>{{ icons.mdiClipboardOutline }}</v-icon>
              </v-chip>
            </v-list-item-subtitle>
            <v-list-item-subtitle>
              <v-chip
      ...
      <v-list>
        <v-list-item>
          <v-chip-group column>
            <v-chip :href="repoUrl('?tab=repositories')" target="_blank">
              repos: {{ $auth.user.public_repos }}
              <v-icon right small>
                {{ icons.mdiOpenInNew }}
              </v-icon>
            </v-chip>
            <v-chip :href="repoUrl('?tab=followers')" target="_blank">
              followers: {{ $auth.user.followers }}
              <v-icon right small>
                {{ icons.mdiOpenInNew }}
              </v-icon>
            </v-chip>
            <v-chip :href="repoUrl('?tab=following')" target="_blank">
