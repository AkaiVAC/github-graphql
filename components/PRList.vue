    ...
    <template v-for="pr in prs.pullRequests.nodes">
      <v-card :key="pr.id" outlined class="mb-2">
        <v-list-item :key="pr.id">
          <a :href="pr.author.url" target="_blank" title="Open author's URL">
          ...
          <v-list-item-content>
            ...
            <v-list-item-subtitle>
              <v-chip small label @click="copyText(pr.id)" class="transparent"
                >ID:&nbsp;{{ pr.id }}
                <v-icon right small>{{ icons.mdiClipboardOutline }}</v-icon>
              </v-chip>
            </v-list-item-subtitle>
              ...
              <v-chip-group column>
                ...
                <div>
                  ...
                  <v-chip
                    ...
                    link
                    class="mr-2 error"
                    @click.stop="$accessor.githubStore.CLOSE_PR(pr.id)"
                  >
                    Close
                  </v-chip>
                </div>
